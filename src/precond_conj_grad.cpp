...
void print_matrix(vector<vector<D_TYPE>> M) {
    ...
    string filename = "PCCG data " + to_string(a) + ".txt";
...
// returns the input matrix multiplied by the inverse of the jacobi precondition xnew = inv(M)*x
vector<vector<D_TYPE>> matvec_M(vector<vector<D_TYPE>> x, D_TYPE C) {
    size_t n = x.size();
    for (int i=0; i<n; i++) {
        for (int j=0; j<n; j++) {
            x[i][j] = (1/(1 + 4*C)) * x[i][j];
...
void solver() {
    ...
    vector<D_TYPE> arr(N);
    vector<vector<D_TYPE>> b(N, vector<D_TYPE>(N,0)), r, p, y, z;
